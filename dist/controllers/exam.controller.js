...
export class ExamController {
    ...
    async getExams(req, res) {
        try {
            ...
            const result = await examService.getExams(page, pageSize, statusParam || undefined, classIdParam || undefined, subjectIdParam || undefined, typeParam || undefined, searchParam || undefined);
            res.json({
                success: true,
     ...
     */
    async getExamWithResults(req, res) {
        try {
            const id = String(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id);
            if (!id || !id.trim()) {
                throw new ValidationError("Exam ID is required");
