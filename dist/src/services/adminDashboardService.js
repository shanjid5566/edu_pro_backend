...
class AdminDashboardService {
    ...
    async getOverallStats() {
        try {
            const [totalStudents, totalTeachers, totalClasses, totalParents, totalSubjects,] = await Promise.all([
                prisma_1.prisma.student.count(),
                prisma_1.prisma.teacher.count(),
                prisma_1.prisma.class.count(),
                prisma_1.prisma.parent.count(),
    ...
    async getPerformanceBySubject() {
        try {
            const results = await prisma_1.prisma.examResult.groupBy({
                by: ["id"],
                _avg: {
                    marksObtained: true,
                },
            ...
            const subjectsPerformance = await prisma_1.prisma.subject.findMany({
                select: {
                    id: true,
                    name: true,
                    exams: {
            ...
            return subjectsPerformance.map((subject) => {
                const marks = subject.exams.flatMap((exam) => exam.results.map((r) => r.marksObtained));
    ...
    async getDashboardData() {
        try {
            const [stats, attendanceTrend, performance, todayAttendance, recentActivity] = await Promise.all([
