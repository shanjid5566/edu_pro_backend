...
class StudentExamService {
    ...
    async getMyExams(studentId) {
        try {
            ...
            const formatExam = (exam) => ({
                ...
                type: exam.type,
                status: exam.status,
                marksObtained: exam.results.length > 0 ? exam.results[0].marksObtained : null,
                percentage: exam.results.length > 0 && exam.results[0].marksObtained !== null
                    ? Math.round((exam.results[0].marksObtained / exam.totalMarks) * 100)
                    : null,
            });
            return {
                success: true,
    ...
    async getExamDetails(studentId, examId) {
        try {
            ...
            const exam = await prisma_js_1.prisma.exam.findFirst({
                where: {
                    id: examId,
                    classId: student.classId,
                },
...
exports.default = new StudentExamService();
