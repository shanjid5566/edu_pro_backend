...
class AdminAttendanceService {
     ...
     */
    static async getAttendanceStatistics(date) {
        const startOfDay = new Date(date);
        startOfDay.setHours(0, 0, 0, 0);
        const endOfDay = new Date(date);
        endOfDay.setHours(23, 59, 59, 999);
        const stats = await prisma_js_1.prisma.attendance.groupBy({
            by: ["status"],
            where: {
                date: {
                    gte: startOfDay,
        ...
        stats.forEach((stat) => {
            if (stat.status === "PRESENT")
                result.present = stat._count;
            if (stat.status === "ABSENT")
                result.absent = stat._count;
            if (stat.status === "LATE")
                result.late = stat._count;
        });
        if (total > 0) {
     ...
     */
    static async getClasswiseAttendance(date) {
        const startOfDay = new Date(date);
        startOfDay.setHours(0, 0, 0, 0);
        const endOfDay = new Date(date);
        endOfDay.setHours(23, 59, 59, 999);
        // Single aggregated query instead of N+1
        const classAttendance = await prisma_js_1.prisma.attendance.groupBy({
            by: ["classId", "status"],
            where: {
                date: {
                    gte: startOfDay,
     ...
     */
    static async getAllAttendance(page = 1, limit = 10, classId, status) {
        ...
        const [attendance, total] = await Promise.all([
            ...
            prisma_js_1.prisma.attendance.count({ where }),
        ]);
        return {
            data: attendance,
            pagination: {
     ...
     */
    static async markAttendance(classId, attendanceData, markedBy, date) {
        const startOfDay = new Date(date);
        startOfDay.setHours(0, 0, 0, 0);
        const endOfDay = new Date(date);
        endOfDay.setHours(23, 59, 59, 999);
        // Delete existing attendance for this class on this date
        await prisma_js_1.prisma.attendance.deleteMany({
            where: {
                classId,
                date: {
                    gte: startOfDay,
        ...
        const attendance = await prisma_js_1.prisma.attendance.createMany({
     ...
     */
    static async updateAttendance(id, status, markedBy) {
        const attendance = await prisma_js_1.prisma.attendance.update({
            where: { id },
            data: {
                status,
                markedBy,
    ...
    static async deleteAttendance(id) {
        await prisma_js_1.prisma.attendance.delete({
            where: { id },
        });
        return {
     ...
     */
    static async getAttendanceByDateRange(startDate, endDate, classId) {
        const where = {
            date: {
                gte: startDate,
                lte: endDate,
     ...
     */
    static async getStudentAttendanceHistory(studentId, limit = 30) {
        const attendance = await prisma_js_1.prisma.attendance.findMany({
            where: { studentId },
            select: {
                id: true,
     ...
     */
    static async getAttendanceReportByClass(classId, startDate, endDate) {
        const students = await prisma_js_1.prisma.student.findMany({
            where: { classId },
            select: {
                id: true,
                user: {
