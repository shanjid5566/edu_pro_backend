...
], handleValidationErrors, noticeController.getAllNotices.bind(noticeController));
/**
 * @swagger
...
], handleValidationErrors, noticeController.getPinnedNotices.bind(noticeController));
/**
 * @swagger
...
router.get("/category/:category", [
    ...
    query("page").optional().isInt({ min: 1 }).withMessage("Page must be a positive integer"),
    query("pageSize").optional().isInt({ min: 1, max: 100 }).withMessage("PageSize must be between 1 and 100"),
], handleValidationErrors, noticeController.getNoticesByCategory.bind(noticeController));
/**
 * @swagger
...
router.get("/search", [
    ...
    query("page").optional().isInt({ min: 1 }).withMessage("Page must be a positive integer"),
    query("pageSize").optional().isInt({ min: 1, max: 100 }).withMessage("PageSize must be between 1 and 100"),
], handleValidationErrors, noticeController.searchNotices.bind(noticeController));
/**
 * @swagger
 * /api/v1/notices/{id}:
 *   get:
...
], handleValidationErrors, noticeController.getNoticeById.bind(noticeController));
// ========================
...
], handleValidationErrors, noticeController.createNotice.bind(noticeController));
/**
 * @swagger
 * /api/v1/notices/{id}:
 *   put:
...
router.put("/:id", verifyToken, requireRole("admin"), [
    ...
    body("priority").optional().isIn(["NORMAL", "HIGH", "URGENT"]).withMessage("Invalid priority"),
    body("pinned").optional().isBoolean().withMessage("Pinned must be a boolean"),
], handleValidationErrors, noticeController.updateNotice.bind(noticeController));
/**
 * @swagger
