...
class AdminTeacherService {
   ...
   */
  async getAllTeachers(options: TeacherListOptions) {
    try {
  ...
  async createTeacher(payload: CreateTeacherPayload) {
    try {
      ...
      if (payload.subjects && payload.subjects.length > 0) {
        try {
          await Promise.all(
            payload.subjects.map((subjectId) =>
      ...
      if (payload.assignClasses && payload.assignClasses.length > 0) {
        try {
          await Promise.all(
            payload.assignClasses.map((classId) =>
  ...
  async updateTeacher(teacherId: string, payload: UpdateTeacherPayload) {
    try {
      ...
      if (payload.joinDate) updateData.joinDate = payload.joinDate;
   ...
   * Search teachers
   */
  async searchTeachers(query: string, limit: number = 10) {
    try {
      const teachers = await prisma.teacher.findMany({
        where: {
          OR: [
   ...
   */
  async exportTeachersToCSV(options: TeacherListOptions) {
    try {
