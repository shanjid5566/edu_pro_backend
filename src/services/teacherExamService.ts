...
class TeacherExamService {
  ...
  async getClassExams(teacherId: string, classId: string) {
    try {
  ...
  async getExamDetails(teacherId: string, examId: string) {
    try {
      ...
      // Verify teacher is assigned to this class
      const isTeacherAssigned = exam.class.teachers.some(
      ...
      return {
        ...
        data: {
          ...
          markedStudents: exam.results.filter(
  ...
  // Get exams by status
  async getExamsByStatus(teacherId: string, status: string) {
    try {
      const exams = await prisma.exam.findMany({
        where: {
  ...
  async submitExamMarks(
    teacherId: string,
    examId: string,
    marks: Array<{ studentId: string; marksObtained: number | null }>
  ) {
    try {
      const exam = await prisma.exam.findUnique({
        where: { id: examId },
