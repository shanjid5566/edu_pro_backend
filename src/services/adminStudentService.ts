...
class AdminStudentService {
   ...
   */
  async getAllStudents(options: StudentListOptions) {
    try {
      ...
      await prisma.profile.create({
        data: {
          ...
          gender: validGenders.includes(payload.gender?.toUpperCase())
            ? (payload.gender?.toUpperCase() as any)
            : null,
  ...
  async updateStudent(studentId: string, payload: UpdateStudentPayload) {
    try {
      ...
      if (payload.classId) updateData.classId = payload.classId;
      if (payload.section) updateData.section = payload.section;
      if (payload.rollNumber) updateData.rollNumber = payload.rollNumber;
      if (payload.grade) updateData.grade = payload.grade;
   ...
   */
  async exportStudentsToCSV(options: StudentListOptions) {
    try {
