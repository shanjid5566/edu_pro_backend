...
export class DashboardService {
   ...
   */
  async getAttendanceTrend(): Promise<AttendanceTrendData[]> {
    const months = ["Sep", "Oct", "Nov", "Dec", "Jan", "Feb"];
    const attendanceData: AttendanceTrendData[] = [];
   ...
   */
  async getPerformanceBySubject(): Promise<PerformanceBySubject[]> {
   ...
   */
  async getTodaysAttendance(): Promise<TodaysAttendance> {
   ...
   */
  async getClassStatistics(page: number = 1, pageSize: number = 10): Promise<ClassStatisticsResponse> {
   ...
   */
  async getTeacherPerformance(page: number = 1, pageSize: number = 10): Promise<TeacherPerformanceResponse> {
    const skip = (page - 1) * pageSize;
  ...
  async getStudentPerformance(
    ...
    const data = students.map((student) => {
      ...
      if (totalMarks >= 85 && attendance >= 90) performance = "excellent";
      else if (totalMarks >= 75 && attendance >= 80) performance = "good";
      else if (totalMarks >= 60 && attendance >= 70) performance = "average";
