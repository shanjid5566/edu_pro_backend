...
export class SettingService {
  ...
  async getGeneralSettings() {
    try {
      ...
      const generalSettings = {
        ...
        schoolAddress: this.getSettingValue(
          settings,
          "schoolAddress",
   ...
   */
  async updateGeneralSettings(input: GeneralSettingsInput) {
    try {
      // Validate inputs
   ...
   */
  async updateUserPreferences(userId: string, input: UserPreferencesInput) {
    try {
      const user = await db.user.findUnique({
        where: { id: userId },
      });
   ...
   */
  async updateAppearanceSettings(userId: string, input: AppearanceSettingsInput) {
    try {
      const user = await db.user.findUnique({
        where: { id: userId },
      });
   ...
   */
  async bulkUpdateSettings(input: BulkSettingsInput) {
    try {
