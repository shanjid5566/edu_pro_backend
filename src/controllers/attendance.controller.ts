...
export class AttendanceController {
   ...
   */
  async getClassWiseAttendance(req: Request, res: Response): Promise<void> {
    try {
      const dateParam = String(Array.isArray(req.query.date) ? req.query.date[0] : req.query.date || "");
      const date = dateParam ? new Date(dateParam) : new Date();
  ...
  async exportAttendance(req: Request, res: Response): Promise<void> {
    try {
      ...
      const firstPage = await attendanceService.getAttendanceRecords(
        1,
        1,
        classIdParam || undefined,
        studentIdParam || undefined,
  ...
  async markAttendance(req: Request, res: Response): Promise<void> {
    try {
      ...
      const attendance = await attendanceService.markAttendance(
        { studentId, classId, date: new Date(date), status },
        userId
      );
   ...
   */
  async bulkMarkAttendance(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
      if (!userId) {
        throw new ValidationError("User not authenticated");
   ...
   */
  async getStudentReport(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  async getStudentPercentage(req: Request, res: Response): Promise<void> {
    try {
      const studentId = String(Array.isArray(req.params.studentId) ? req.params.studentId[0] : req.params.studentId);
      const startDateParam = String(Array.isArray(req.query.startDate) ? req.query.startDate[0] : req.query.startDate || "");
      const endDateParam = String(Array.isArray(req.query.endDate) ? req.query.endDate[0] : req.query.endDate || "");
      ...
      const percentage = await attendanceService.getStudentAttendancePercentage(
        studentId,
        startDate,
        endDate
      );
   ...
   */
  async getTeacherAttendanceSheet(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
      if (!userId) {
        throw new ValidationError("User not authenticated");
   ...
   */
  async saveTeacherAttendanceSheet(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
      if (!userId) {
        throw new ValidationError("User not authenticated");
   ...
   */
  async getTeacherRecentAttendance(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
      if (!userId) {
        throw new ValidationError("User not authenticated");
