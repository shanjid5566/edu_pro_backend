...
export class ExamController {
   ...
   */
  async getTeacherExams(req: Request, res: Response): Promise<void> {
    try {
      if (!req.user?.id) {
   ...
   */
  async createTeacherExam(req: Request, res: Response): Promise<void> {
    try {
      if (!req.user?.id) {
        throw new ValidationError("User authentication required");
      }
   ...
   */
  async uploadTeacherQuestionPaper(req: Request, res: Response): Promise<void> {
    try {
      if (!req.user?.id) {
        throw new ValidationError("User authentication required");
      }
